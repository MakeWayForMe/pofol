...
(function($) {
    let fileRoot = window.location.href.split('/'),
        ...
        fileName = fileRoot[fileRoot.length-1].includes('view') ? fileRoot[fileRoot.length-1].split('.')[0].split('_')[0] : fileRoot[fileRoot.length-1].split('.')[0];
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            document.querySelector('head title').innerText = nowPageKey + '│유한건강생활';

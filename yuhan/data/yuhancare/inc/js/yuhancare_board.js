...
if(!url.includes('_view.html')) { // 게시판 화면
            ...
            if(data.length <= showContent) { //컨텐츠가 한페이지를 못채울 경우
                $.each(data, function(i, obj){
                    if(boardType === 'table') { //테이블 형식
                        content += `<tr><td class="number" data-name="NO">${obj.no}</td><td><a href="/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${obj.no}">${obj.title}</a></td><td class="date">${obj.date}</td></tr>`;
                        ...
                        content += `<a class="context" href="/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${obj.no}"><div class="imgbox link_icon" style="background-image:url(${obj.img});"></div><div class="title">${obj.title}</div><div class="date">${obj.date}</div></a>`;
                    }
                });
                pageLink = `<a href="/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}?page=1" class="now">1</a>`;
                ...
                if(page === paging + 1) { //마지막 페이지
                    ...
                    if(boardType === 'table') { //테이블 형식
                        for(var i=count;i<count+remainder;i++) {
                            content += `<tr><td class="number" data-name="NO">${data[i].no}</td><td><a href="/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${data[i].no}">${data[i].title}</a></td><td class="date">${data[i].date}</td></tr>`;
                        }
                    } else if(boardType === 'video') { //유튜브링크
                        for(var i=count;i<count+remainder;i++) {
                        ...
                        for(var i=count;i<count+remainder;i++) {
                            content += `<a class="context" href="/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${data[i].no}"><div class="imgbox link_icon" style="background-image:url(${data[i].img});"></div><div class="title">${data[i].title}</div><div class="date">${data[i].date}</div></a>`;
                        }
                    }
                } else { //마지막 페이지가 아닐때
                    if(boardType === 'table') { //테이블 형식
    ...
    const key = Number(nowUrl.searchParams.get('key'));
        ...
        success: function(data) {
            ...
            if(nowContent < 1) {
                if(key === 1 ) {
                    ...
                    $('a.next').text('다음글이 없습니다').on('click',(e) => e.preventDefault()).addClass('disabled');
                    $('a.prev').text(data[nowContent + 1].title).attr('href',`/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${key - 1}`).siblings('span.date').text(data[nowContent + 1].date);
                }
            } else if(key === 1) {
                $('a.next').text(data[nowContent - 1].title).attr('href',`/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${key + 1}`).siblings('span.date').text(data[nowContent - 1].date);
                $('a.prev').text('이전글이 없습니다').on('click',(e) => e.preventDefault()).addClass('disabled');
            } else {
                $('a.next').text(data[nowContent - 1].title).attr('href',`/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${key + 1}`).siblings('span.date').text(data[nowContent - 1].date);
                $('a.prev').text(data[nowContent + 1].title).attr('href',`/portfolio/yuhan/data/yuhancare/${urlPath}/${htmlPath}_view.html?key=${key - 1}`).siblings('span.date').text(data[nowContent + 1].date);
